...
const LEDGER_CSS = join(
  process.cwd(),
  "src/features/operator/operator-mini-ledger.css",
);

function progressAfterRule(css: string): string {
  const start = css.indexOf(".mini-action-journal__progress::after {");
  expect(
    start,
    "правило .mini-action-journal__progress::after не найдено",
  ).toBeGreaterThan(-1);
  const end = css.indexOf("}", start);
  return css.slice(start, end);
}

describe("mini action journal progress bar", () => {
