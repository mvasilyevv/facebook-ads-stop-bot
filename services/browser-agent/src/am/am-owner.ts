...
export function parseOwnerTags(raw: string | null | undefined): string[] {
  if (!raw) return [];
  return raw
    .replace(/;/g, ',')
    .split(',')
...
function ownerTagPattern(tagLower: string): RegExp {
...
export function campaignMatchesOwner(
  campaignName: string,
...
export function resolveOwnerCampaignIds(
  campaigns: Array<{ id: string; name?: string }>,
  ownerTag: string | null | undefined,
): string[] {
  if (!parseOwnerTags(ownerTag).length) return [];
