...
export async function applyAdsTableColumnWidthPreset(page: Page): Promise<ColumnWidthApplyResult> {
  ...
  try {
    const result = await page.evaluate(async (columnTargets: ColumnWidthTarget[]) => {
      ...
      function targetLeftPx(targetIndex: number): number {
      ...
      function collectHorizontalScrollables(): HTMLElement[] {
      ...
      function scrollRight(): { moved: boolean; atRight: boolean; scrollLeft: number; maxScrollLeft: number } {
      ...
      function applyVisibleWidths(): {
        ...
        for (const entry of matchedEntries) {
          adjustedCells += updateHeaderChain(
            entry.headerNode,
            entry.target.widthPx,
            targetLeftPx(entry.targetIndex),
          );
        }
        ...
        for (const rowNode of rowNodes) {
          ...
          const usedCells = new Set<HTMLElement>([cells[0]]);
          for (const headerRect of headerRects) {
            let best: { cell: HTMLElement; delta: number } | null = null;
            for (const cellRect of cellRects) {
              if (usedCells.has(cellRect.cell)) continue;
              const delta = Math.abs(cellRect.left - headerRect.left);
              if (!best || delta < best.delta) best = { cell: cellRect.cell, delta };
            }
            if (!best || best.delta > 96) continue;
          ...
          if (customizeCell) {
            ...
            if (isBodyRow) customizeCell.style.left = px(customizeLeft);
            adjustedCells += 1;
          }
        }
      ...
      for (let pass = 0; pass < MAX_HORIZONTAL_PASSES; pass++) {
        ...
        for (const column of step.matchedColumns) matchedColumns.add(column);
        for (const key of step.matchedKeys) matchedKeys.add(key);

        if (matchedKeys.size >= columnTargets.length) break;
        ...
        if (!scroll.moved || scroll.atRight) {
          if (scroll.atRight) {
            ...
            for (const column of finalStep.matchedColumns) matchedColumns.add(column);
            for (const key of finalStep.matchedKeys) matchedKeys.add(key);
          }
          break;
        }
        await wait(SCROLL_SETTLE_MS);
      ...
      for (const column of leftStep.matchedColumns) matchedColumns.add(column);
      for (const key of leftStep.matchedKeys) matchedKeys.add(key);
