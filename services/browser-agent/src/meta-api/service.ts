...
export function createMetaApiServiceHandlers(
  ...
  async function checkMetaApiHealthHandler(call: any, callback: any): Promise<void> {
    ...
    try {
      ...
      //
      // Названный кабинет здесь — поручение подготовить вкладку, а не уточнение
      // вопроса. Поэтому его называет только явное обслуживание канала
      // (ensure-cdp) и money-предполёт; проба готовности ходит без кабинета и
      // ничего не готовит.
