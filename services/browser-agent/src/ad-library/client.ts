 ...
 */
export async function searchAdsBatch(
  context: BrowserContext,
  params: SearchAdsBatchParams,
): Promise<SearchAdsBatchResult> {
  const t0 = Date.now();
  ...
  try {
    page = await context.newPage();
    const bootstrapResult = await bootstrapPage(
      page,
      params.country,
      params.perQueryTimeoutMs,
    );
    if (bootstrapResult.error) {
...
): Promise<{ error?: { code: number; type: string; message: string } }> {
...
async function fetchAdsWithPagination(
  ...
  for (let pageIdx = 0; pageIdx < maxPages; pageIdx++) {
    ...
    const connection =
      parsed?.data?.ad_library_main?.search_results_connection ||
    ...
    const edges: any[] = Array.isArray(connection.edges) ? connection.edges : [];
    for (const edge of edges) {
      const collated: any[] = edge?.node?.collated_results || edge?.node?.collatedResults || [];
...
async function executeAdLibraryGraphQL(args: {
  ...
  const rev =
    matchFirst([/"__spin_r":(\d+)/, /"server_revision":(\d+)/]) || '1033837939';
  const spinT =
    matchFirst([/"__spin_t":(\d+)/]) || String(Math.floor(Date.now() / 1000));
  ...
  body.set('__hsi', hsi);
  if (dyn) body.set('__dyn', dyn);
  if (csr) body.set('__csr', csr);
  if (hsdp) body.set('__hsdp', hsdp);
  if (hblp) body.set('__hblp', hblp);
  if (sjsp) body.set('__sjsp', sjsp);
 ...
 */
async function captureUiGraphQLRequest(
  context: BrowserContext,
  params: SearchAdsParams,
): Promise<SearchAdsResult> {
  const t0 = Date.now();
    ...
    page.on('request', (req) => {
      try {
        ...
        for (const pair of postData.split('&')) {
          ...
          if (k) parsed[decodeURIComponent(k)] = decodeURIComponent(v || '').slice(0, 600);
        }
        captures.push({
 ...
 */
export async function checkAdLibraryHealth(
  context: BrowserContext | null | undefined,
): Promise<AdLibraryHealthResult> {
  if (!context) {
