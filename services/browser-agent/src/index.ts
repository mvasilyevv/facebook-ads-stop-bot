...
import {
  startBrowserAgentMetricsServer,
  type BrowserAgentMetricsServer,
} from "./metrics.js";
import {
...
export function validateBrowserCapabilitySecrets(
  ...
  const authorityUrl = String(environment.BROWSER_AUTHORITY_CONSUME_URL || "");
  const maintenanceAuthorityUrl = String(
    environment.BROWSER_MAINTENANCE_CONSUME_URL || "",
  );
  const valid = (value: string): boolean => /^[A-Za-z0-9_-]{48,}$/.test(value);
...
export function verifyMaintenanceCapabilitySignature(
  ...
  const secret =
    options.secret ?? process.env.BROWSER_MAINTENANCE_CAPABILITY_SECRET ?? "";
  ...
  try {
    ...
    if (!abort.controller.signal.aborted)
      callback(null, {
        ...
        initial_page_url:
          canonicalAdsManagerUrl(session.primaryPage?.url()) || "",
      });
  } catch (err: any) {
    if (!abort.controller.signal.aborted)
      callback({
...
export function createRecoverBrowserProfileHandler(
  ...
  return async function recoverBrowserProfileUnderMaintenance(
    call: any,
    callback: any,
  ) {
    try {
      ...
      callback({
        ...
        message: String(
          err?.message || "Browser maintenance capability rejected",
        ),
      });
      return;
    }
    ...
    const abort = lifecycleAbort(
      call,
      Number(call.request.capability_expires_at),
    );
    try {
      const req = call.request;
      ...
      if (!abort.controller.signal.aborted) {
        const code =
          err instanceof MaintenanceCapabilityConsumeDeniedError
        ...
        callback({
          code,
          message: String(
            err?.message || "Browser maintenance recovery failed",
          ),
        });
      }
    } finally {
    ...
    for (const actId of actIds) {
      ...
      try {
        ...
        results.push({
          ad_account_id: actId,
          opened: true,
          url: canonicalAdsManagerUrl(scanPage.url()) || "",
          error: "",
        });
      } catch (e: any) {
        if (abort.controller.signal.aborted) {
...
async function runScanCycle(call: any) {
  ...
  try {
    ...
    const campaignIds: string[] = Array.isArray(req.campaign_ids)
      ? req.campaign_ids
      : [];
    ...
    const scan = await withPageRoleLock(
      ...
      async () => {
        ...
        if (
          result.diagnostics.authExpired &&
          !result.diagnostics.loginRequired
        ) {
          console.warn(
            "[scan][am] access_token протух (190) → re-sniff + retry",
          );
        ...
        if (netFail && shouldHealNow(session, Date.now())) {
          invalidateGraphContext(req.session_id, actId);
          const healed =
            await sessionManager.reloadPageAfterNetworkFailureWithinRoleLock(
              req.session_id,
              {
                role: "scan",
                actId,
          ...
          console.warn(
            `[scan][am] page reload: ${healed.action} (ok=${healed.ok})`,
          );
        }
...
function main() {
  ...
  const browserSessionService =
    browserSessionProto.fb_agent.browser_session.v1.BrowserSessionService;
  ...
  server.bindAsync(
    ...
    async (error, port) => {
      ...
      try {
        ...
        console.error(
          "Не удалось запустить metrics-сервер Browser Agent:",
          metricsError,
        );
      ...
      console.log(
        `Browser Agent слушает gRPC :${port} и metrics :${metricsServer.port}`,
      );
      server.start();
    },
  );
