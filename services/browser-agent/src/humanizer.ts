...
export function generateHumanProfile(): HumanProfile {
  return {
...
function _perlinDelay(baseMin: number, baseMax: number, t: number, seed: number): number {
...
export async function _resolveScrollAnchor(page: Page): Promise<[number, number] | null> {
  try {
...
async function _resolveViewport(page: Page): Promise<{ width: number; height: number }> {
...
export async function humanMove(
  ...
  targetX: number,
  targetY: number,
  options?: { currentPos?: [number, number]; profile?: HumanProfile },
): Promise<void> {
...
export async function humanClick(
  ...
  options?: { doubleCheckPause?: boolean; profile?: HumanProfile },
): Promise<void> {
  const prof = options?.profile ?? generateHumanProfile();
...
export async function humanScrollToFind(
  page: Page,
  selector: string,
  options?: { maxSteps?: number; stepPx?: number; profile?: HumanProfile },
