...
export class DuplicateAdsetStep extends BaseStep<DuplicateAdsetInput, void> {
  ...
  protected async run(_s: StepState, input: DuplicateAdsetInput): Promise<void> {
    const node = findTreeNodeByName('adset', input.sourceName);
    if (!node) throw new Error(`Ad set "${input.sourceName}" не найден в дереве`);
    const menu =
