...
exports.VideoUploadSession = void 0;
 ...
 */
async function uploadImage(page, params) {
    ...
    const evalArgs = {
        ...
        base64Data: base64,
        timeoutMs,
        apiVersion: client_js_1.META_API_VERSION,
    };
    try {
 ...
 */
async function uploadImageFromUrl(page, params) {
    const t0 = Date.now();
    const evalArgs = {
        adAccountId: params.adAccountId,
        imageUrl: params.imageUrl,
    ...
    try {
        const result = await page.evaluate(async (args) => {
            ...
            if (args.name)
                qp.set('name', args.name);
            const endpoint = `https://graph.facebook.com/${args.apiVersion}/${args.adAccountId}/adimages?${qp.toString()}`;
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), args.timeoutMs);
            try {
...
class VideoUploadSession {
    ...
    constructor(page, params) {
        ...
        this.apiVersion = client_js_1.META_API_VERSION;
    ...
    async start() {
        ...
        const result = await this.page.evaluate(async (args) => {
            const match = document.documentElement.innerHTML.match(/EAA[A-Za-z0-9_-]{100,}/);
            if (!match)
                return { ok: false, error: 'TOKEN_NOT_FOUND_IN_PAGE', uploadSessionId: '', videoId: '', startOffset: 0, endOffset: 0 };
            const token = match[0];
            const form = new FormData();
            ...
            try {
                ...
                try {
                    ...
                    return { ok: false, error: `Невалидный JSON: ${text.slice(0, 300)}`, uploadSessionId: '', videoId: '', startOffset: 0, endOffset: 0 };
                }
                if (parsed?.error)
                    return { ok: false, error: `GRAPH_ERROR_${parsed.error.code}: ${parsed.error.message}`, uploadSessionId: '', videoId: '', startOffset: 0, endOffset: 0 };
                return {
                    ok: true,
                    error: '',
    ...
    async transfer(chunkBytes) {
        ...
        const result = await this.page.evaluate(async (args) => {
            const match = document.documentElement.innerHTML.match(/EAA[A-Za-z0-9_-]{100,}/);
            if (!match)
                return { ok: false, error: 'TOKEN_NOT_FOUND_IN_PAGE', startOffset: 0, endOffset: 0 };
            const token = match[0];
            const binary = atob(args.base64Data);
            const bytes = new Uint8Array(binary.length);
            for (let i = 0; i < binary.length; i++)
            ...
            try {
                ...
                try {
                    ...
                    return { ok: false, error: `Невалидный JSON: ${text.slice(0, 300)}`, startOffset: 0, endOffset: 0 };
                }
                if (parsed?.error)
                    return { ok: false, error: `GRAPH_ERROR_${parsed.error.code}: ${parsed.error.message}`, startOffset: 0, endOffset: 0 };
                return {
                    ok: true,
                    error: '',
                    startOffset: Number(parsed.start_offset ?? 0),
    ...
    async finish() {
        ...
        const result = await this.page.evaluate(async (args) => {
            const match = document.documentElement.innerHTML.match(/EAA[A-Za-z0-9_-]{100,}/);
            if (!match)
                return { ok: false, error: 'TOKEN_NOT_FOUND_IN_PAGE', success: false, videoId: '' };
            const token = match[0];
            const form = new FormData();
            ...
            try {
                ...
                try {
                    ...
                    return { ok: false, error: `Невалидный JSON: ${text.slice(0, 300)}`, success: false, videoId: '' };
                }
                if (parsed?.error)
                    return { ok: false, error: `GRAPH_ERROR_${parsed.error.code}: ${parsed.error.message}`, success: false, videoId: '' };
                return {
                    ok: true,
                    error: '',
    ...
    get sessionId() { return this.uploadSessionId; }
    get id() { return this.videoId; }
    get isStarted() { return this.started; }
    get isFinished() { return this.finished; }
    get nextOffset() { return this.currentOffset; }
}
exports.VideoUploadSession = VideoUploadSession;
