...
exports.humanPressKey = humanPressKey;
exports.humanTypeText = humanTypeText;
...
async function humanPressKey(page, key, options) {
    const prof = options?.profile ?? generateHumanProfile();
...
async function readEditableText(element) {
...
async function editableTextMatches(element, actual, expected) {
    if (actual === expected)
        return true;
...
async function selectEditableContents(element) {
    await element.evaluate((node) => {
        ...
        if (editable.isContentEditable && typeof document !== 'undefined') {
...
async function clearEditableText(page, element, profile) {
    ...
    if (!(await readEditableText(element)))
        return;
...
async function typeTextWithHumanDelay(page, text, profile, perCharDelayRange) {
...
async function humanTypeText(page, element, text, options) {
    const prof = options?.profile ?? generateHumanProfile();
