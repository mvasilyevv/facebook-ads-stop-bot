...
async def _enqueue_standalone_campaign_unknown(
    ...
    *,
    ...
    # Доказанный отказ до отправки в Facebook: последний запрос до Meta не дошёл.
    # Про содержимое кабинета это не говорит ничего сверх перечня — залитое видео
    # живёт в медиатеке и в перечень не попадает, поэтому карточка утверждает только
    ...
    await notify_recurring_incident_in_transaction(
        ...
        summary=(
            "Создалась часть объектов, последний запрос до Facebook не дошёл."
            if pre_dispatch
            else (
