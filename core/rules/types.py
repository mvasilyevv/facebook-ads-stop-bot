...
class RuleContext:
    ...
    # Sanity-минимум для guardrail-правил (cpc/cpl/cpr при 0 событий): отсекает только
    # 1-2-показный выброс (один случайно дорогой показ CPM мог дать spend выше порога
    # ещё до реального открута). Дальше работает обычный порог spend>=stop_threshold,
    # который сам фильтрует мизерный спенд. Порог 3 (а не 10): расход без события выше
    # стоп-порога — money-сигнал, ждать 10 показов незачем (вменённый CPC уже дорогой).
    guardrail_min_impressions: int = 3
