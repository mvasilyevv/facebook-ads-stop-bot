 ...
 * `/api/operator/actions` отдаёт в `reason` только то, что воркер записал при
 * завершении задачи (`task_action_reason`), а отмену скана никто причиной не
 * снабжает — там `null`. Строить подпись кнопки из этого поля значит зависеть
 * от того, дошла ли причина; «что чинить» уже живёт рядом в секции внимания
