...
async def _sync_vision_profiles(repo: BrowserRepository) -> None:
    ...
    runtime_settings = await resolve_service_settings(repo.session, base_settings=base_settings)
    settings = build_effective_settings(base_settings, runtime_settings)
    adapter = build_adapter(settings)
    try:
...
async def start_session(
    ...
    runtime_settings = await resolve_service_settings(session, base_settings=get_settings())
    settings = build_effective_settings(get_settings(), runtime_settings)
    manager = build_session_manager(settings)
    repo = BrowserRepository(session)
    try:
        launch_result = await manager.ensure_profile_started(profile_id)
