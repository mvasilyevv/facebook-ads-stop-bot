...
class CampaignConfigIn(BaseModel):
    ...
    page_id: str
    pixel_id: str
    # Явное подтверждение оператора, что pixel_id отличается от offer.pixel_id
    # НАМЕРЕННО (кабинет может держать несколько валидных пикселей). По умолчанию
    # False — сервер отказывает на расхождении, чтобы тихая подмена пикселя не
    # ушла в Meta незамеченной. Не участвует в доменном конфиге и идемпотентности:
    # это процедурное согласие с проверкой, а не часть определения кампании.
