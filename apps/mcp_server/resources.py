        ...
        items.append(
            {
                ...
                "fb_ad_id": fb_ad_id,
                "ad_name": ad_name,
                # Явная identity кабинета; null означает неизвестный источник.
...
def _render_schema_overview() -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "- MCP не предлагает и не исполняет изменения рекламы. «Посмотри, что "
        "отключить» = read-only анализ и рекомендация для оператора.",
        "",
        "## Рекомендованный порядок работы",
        "",
        "1. Контекст: прочитай ресурсы `offers` (коды + кабинеты) и `recent-alerts`.",
        "2. Диагностика money-actions → `get_ad_action_status`; состояние control plane "
        "проверяй в operator snapshot, а process liveness — в Prometheus.",
        ...
        "4. Изменения рекламы оператор выполняет вне MCP после отдельного решения.",
        "",
        "## Категории",
        "",
    ...
    for risk in (RiskLevel.READ_ONLY, RiskLevel.CREATIVE):
