...
CHANNEL_RESTART = "fb_agent:worker:restart:enable_worker"
...
async def main_loop(
    *,
    ...
    """Основной цикл enable_worker с поддержкой Redis-сигнала restart.
    ...
    if redis_factory is not None:
        ...
        try:
            ...
            logger.warning("enable_worker: Redis недоступен — pubsub-listener отключён")
    ...
    try:
        ...
        if redis_client is not None:
            ...
            async def _on_restart(_payload: dict) -> None:
                ...
                logger.info("enable_worker: получен сигнал restart по каналу %s", CHANNEL_RESTART)
                stop_event.set()
