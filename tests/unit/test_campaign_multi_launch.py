...
def test_idempotency_key_is_scoped_by_account() -> None:
    ...
    assert _compute_campaign_idempotency_key(_config("111")) != _compute_campaign_idempotency_key(
        _config("222")
    )
...
async def test_account_failure_does_not_cancel_later_accounts() -> None:
    ...
    results = await _run_launches_independently(
        ("111", "222", "333"),
        launch_one,
    )

    assert called == ["111", "222", "333"]
    assert [(result.key, result.value) for result in results if result.error is None] == [
        ("111", "run-111"),
        ("333", "run-333"),
    ]
    assert [(result.key, str(result.error)) for result in results if result.error] == [
...
async def test_repeated_accounts_are_not_executed_twice_inside_one_request() -> None:
    ...
    results = await _run_launches_independently(("111", "111", "222"), launch_one)

    assert calls == ["111", "222"]
    assert [result.key for result in results] == ["111", "222"]
