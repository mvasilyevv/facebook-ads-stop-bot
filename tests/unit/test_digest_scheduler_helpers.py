...
def test_is_in_send_window_at_boundary() -> None:
    ...
    assert is_in_send_window(now, DigestWindow(hour=9, minute=0)) is True
...
def test_is_in_send_window_before() -> None:
    ...
    assert is_in_send_window(now, DigestWindow(hour=9, minute=0)) is False
...
def test_is_in_send_window_with_minutes() -> None:
    window = DigestWindow(hour=9, minute=30)
...
def test_is_in_send_window_rejects_naive() -> None:
    ...
    with pytest.raises(ValueError):
        is_in_send_window(naive, DigestWindow(hour=9, minute=0))
