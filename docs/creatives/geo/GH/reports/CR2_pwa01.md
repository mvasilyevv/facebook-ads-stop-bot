...
> Распределение видимых: 11×5 + 2×4 + 2×2 + 1×1; ответ дева на 9/16 (все 3 негатива + 6 позитивов); агрегат-headline 4.9 / 341.
...
**Подход (как в образце):** одно широкое hero-арт → нарезка на **3 вертикальных среза 9:16** = консистентная
панорама в карусели (свайп 1→2→3 читается как одна сцена: дорога/палитра/лого тянутся сквозь срезы). Hero
сгенерён в syntx (**GPT Image 2, референс = реальная иконка** → персонаж/лого/стиль взяты прямо с неё; 2K
16:9; текст без опечаток). 2 варианта, выбран **v1** (одиночная связная курица); v2 отклонён (двоилась голова
курицы — AI-артефакт). FEATURE = полный hero 16:9; 3 скрина = центр-кроп **1500×888** (5:3, как ширина
баннера-образца) нарезан на 3×500×888.
...
| FEATURE/постер | **16:9 1024×576** | полный hero v1: курица бежит через дорогу мимо машины, бейджи «6x»/«20x», «BIG WIN», монеты, лого CHICKEN ROAD 2, фиолет/золото | ✅ `CR2_FEATURE_final.jpg` |
| SCREEN 1 | 9:16 500×888 | левый срез hero: бейдж «6X» + красная машина на дороге + монеты | ✅ `CR2_SCREEN1_final.jpg` |
| SCREEN 2 | 9:16 500×888 | центр hero: курица + логотип «CHICKEN ROAD» (геймплей-герой) | ✅ `CR2_SCREEN2_final.jpg` |
| SCREEN 3 | 9:16 500×888 | правый срез hero: «20X» + «BIG WIN» starburst + монеты | ✅ `CR2_SCREEN3_final.jpg` |
...
> Старые ассеты от 1xBet-плана — в `data/syntx_out/_archive_GH_CR2_old/` (не удалены). Сырой hero (оба
> варианта 2048×1152) — в `/tmp` (не коммитим; при нужде регенерим из §7-промпта).
