...
> ⛔ **УСТАРЕЛО / АНТИ-ПРИМЕР (помечено 2026-06-09).** Этот отчёт привёл к провальному заливу
> Aviator на GH (0 FTD, слот заморожен → `slots/AVI.yaml`, локомотив сменён на Chicken Road).
> **Ключевые ошибки отчёта — НЕ повторять:** (1) выводы про `gh_pidgin_touch`/football как
> «свободную нишу / наш дифференциатор» («конкуренты не используют → сделаем мы») — это red flag
> «свободного слота», а не находка (рынок льёт на English — урок подтвердился: полный Pidgin не
> сработал); (2) перенос гипотез из KE как основа плана (SOP R6 теперь это запрещает). Сохранён
> как урок, НЕ как руководство к действию.
