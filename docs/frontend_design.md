...
Спецификация дизайн-системы и страничных макетов для нового фронта FB Stop Bot. Документ обязателен для всех, кто пишет UI-код в `frontend/` (директория создаётся отдельным PR — этот документ её опережает).
...
### 0.5 Язык UI (ОБЯЗАТЕЛЬНО)

**Весь UI — на русском.** Заголовки (`Панель.`, `Объявления.`, `Офферы.`, `История.`, `Настройки.`, `Черновики.`), навигация, eyebrow-маркеры (`ОБЗОР`, `УПРАВЛЕНИЕ · КОНТРОЛЬ`), кнопки, лейблы, состояния, empty/error-тексты.

**Исключение — ad-ops жаргон латиницей** (арбитражники так говорят, перевод режет глаз): `spend`, `CTR`, `CPM`, `CPL`, `CPC`, `CPA`, `ROAS`, `CR`, `offer`, `draft`, `creative`. Имена сервисов (`Observer`, `Vision`, `Telegram`) — тоже латиницей.

**FSM-состояния — русские лейблы** при отображении: `normal→Норма`, `warning_sent→Предупреждение`, `stop_sent→Стоп`, `claimed→В работе`, `disabled→Отключено`. Сами значения в API/коде — не трогать.

**Не переводить:** имена полей API, значения enum в данных (`alert_state`/`task_type`/`status`), ключи Redis, `fb_ad_id`, код-сущности.
