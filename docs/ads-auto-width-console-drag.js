(async () => {
  if (window.__fbAutoWidthDragRunning) {
    console.warn('[auto-width-drag] Уже выполняется предыдущий запуск.');
    return window.__fbAutoWidthDragLastResult || null;
  }
  window.__fbAutoWidthDragRunning = true;
  ...
  function findColumnCell(headerNode) {
    ...
    for (let node = headerNode.parentElement; node instanceof HTMLElement; node = node.parentElement) {
      ...
      if (rect.width > 20 && rect.height > 20 && (node.style.width || node.classList.contains('_4lg0'))) return node;
    }
    return null;
  }
  ...
  function clippingBounds(node) {
    ...
    for (let parent = node.parentElement; parent; parent = parent.parentElement) {
      ...
      if (clipsX) {
        left = Math.max(left, rect.left + 3);
        right = Math.min(right, rect.right - 3);
      }
      if (clipsY) {
        top = Math.max(top, rect.top + 3);
        bottom = Math.min(bottom, rect.bottom - 3);
      }
    }
    return { left, right, top, bottom };
  }

  function collectCandidates(processedKeys = new Set()) {
    const result = [];
    const seen = new Set();
    ...
    for (const headerNode of headers) {
      ...
      if (!target || processedKeys.has(target.key) || seen.has(target.key)) continue;
      ...
      const x = sepRect.left + sepRect.width / 2;
      const y = sepRect.top + sepRect.height / 2;
      const bounds = clippingBounds(separator);
      const visible = cellRect.width > 20 && cellRect.height > 20
        && sepRect.width > 0 && sepRect.height > 0
        && x >= bounds.left && x <= bounds.right && y >= bounds.top && y <= bounds.bottom;
      if (!visible) continue;
      seen.add(target.key);
      result.push({ target, cell, separator, cellRect, sepRect, x, y, bounds });
    }
    return result.sort((a, b) => a.cellRect.left - b.cellRect.left);
  }
  ...
  function findHorizontalScroller() {
    ...
    function add(node) {
      ...
      candidates.push({
        node,
        maxScrollLeft,
        score: maxScrollLeft + (containsHeader ? 100000 : 0) + Math.round(rect.width + rect.height),
      });
    }
    const anchors = [
      header,
  ...
  async function dragSeparator(candidate, delta) {
    const startX = candidate.x;
    const startY = candidate.y;
    const minX = candidate.bounds.left + 8;
    const maxX = candidate.bounds.right - 8;
    const endX = Math.max(minX, Math.min(maxX, startX + delta));
    const steps = Math.max(4, Math.min(CONFIG.dragSteps, Math.ceil(Math.abs(endX - startX) / 12)));

    candidate.separator.scrollIntoView({ block: 'nearest', inline: 'nearest' });
    await sleep(40);

    dispatchPointer(candidate.separator, 'pointerover', startX, startY, 0);
    dispatchMouse(candidate.separator, 'mouseover', startX, startY, 0);
    dispatchPointer(candidate.separator, 'pointermove', startX, startY, 0);
    dispatchMouse(candidate.separator, 'mousemove', startX, startY, 0);
    dispatchPointer(candidate.separator, 'pointerdown', startX, startY, 1);
    dispatchMouse(candidate.separator, 'mousedown', startX, startY, 1);

    for (let i = 1; i <= steps; i += 1) {
      const x = startX + ((endX - startX) * i) / steps;
      dispatchPointer(document, 'pointermove', x, startY, 1);
      dispatchMouse(document, 'mousemove', x, startY, 1);
      await sleep(16);
    }

    dispatchPointer(document, 'pointerup', endX, startY, 0);
    dispatchMouse(document, 'mouseup', endX, startY, 0);
    dispatchPointer(candidate.separator, 'pointerout', endX, startY, 0);
    dispatchMouse(candidate.separator, 'mouseout', endX, startY, 0);
    await sleep(CONFIG.dragSettleMs);

    return { requestedEndX: Math.round(startX + delta), actualEndX: Math.round(endX), clamped: Math.abs(endX - (startX + delta)) > 2 };
  }

  async function scrollScroller(scroller, direction, small = false) {
    if (!(scroller instanceof HTMLElement)) return false;
    const before = scroller.scrollLeft;
    const step = small ? CONFIG.smallScrollPx : Math.max(160, Math.round(scroller.clientWidth * 0.7));
    const max = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
    scroller.scrollLeft = Math.max(0, Math.min(max, before + direction * step));
    scroller.dispatchEvent(new Event('scroll', { bubbles: true }));
    await sleep(CONFIG.settleMs);
    return Math.abs(scroller.scrollLeft - before) > 2;
  }
  ...
  try {
    ...
    for (let pass = 1; pass <= CONFIG.maxPasses; pass += 1) {
      ...
      for (let guard = 0; guard < TARGETS.length + 2; guard += 1) {
        ...
        const before = candidate.cell.getBoundingClientRect().width;
        const delta = Math.round(target.widthPx - before);
        let drag = null;
        let status = 'skip';
        ...
        if (Math.abs(delta) <= CONFIG.tolerancePx) {
          ...
          drag = await dragSeparator(candidate, delta);
          const after = candidate.cell.getBoundingClientRect().width;
          const matched = Math.abs(target.widthPx - after) <= CONFIG.tolerancePx;
          if (matched) {
            processed.add(target.key);
            status = 'drag-ok';
          } else if (drag.clamped && after > target.widthPx + CONFIG.tolerancePx) {
            await scrollScroller(scroller, -1, false);
            status = 'needs-left-scroll';
          } else if (drag.clamped && after < target.widthPx - CONFIG.tolerancePx) {
            await scrollScroller(scroller, 1, true);
            status = 'needs-right-scroll';
          } else {
            processed.add(target.key);
            status = 'drag-no-width-change';
          }
        ...
        rows.push({
          ...
          delta,
          status,
          clamped: Boolean(drag?.clamped),
        });
      }

      if (processed.size >= TARGETS.length) break;
      const moved = await scrollScroller(scroller, 1, false);
      if (!moved && !changedThisPass) break;
    }

    const missing = TARGETS.filter((target) => !processed.has(target.key)).map((target) => target.title);
    const result = { ok: missing.length === 0, processed: processed.size, expected: TARGETS.length, missing, rows };
    console.table(rows);
    if (missing.length) console.warn('[auto-width-drag] Не найдены/не обработаны:', missing);
    else console.info('[auto-width-drag] Все колонки обработаны через separator drag.');
    window.__fbAutoWidthDragLastResult = result;
    return result;
  } finally {
    window.__fbAutoWidthDragRunning = false;
  }
})();
