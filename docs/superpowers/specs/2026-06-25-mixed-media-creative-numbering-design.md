...
## Нейминг кампаний (решение 2026-06-25, в ходе реализации)

Удаление `kind` убирает сегмент типа (`static`/`video`) из имён кампаний/adset'ов
(`{byer} | {offer} | <type> | adset.pro | {date}`). Это ломает различимость двух
кампаний одного оффера/даты (имена совпадут). Решение пользователя:

- Сегмент типа убрать.
- Добавить **опциональную произвольную метку per-campaign** (свободный ввод в шаге 4).
  Если задана — добавляется в КОНЕЦ имени; если пусто — ничего.
- Имя кампании: `{byer} | {offer} | adset.pro | {date}` + (` | {label}` если задана).
- Имя adset'а: `{byer} | {offer} | s{i} | {date}` + (` | {label}` если задана) — метка
  нужна и в adset'е, иначе `s1` двух кампаний совпадут.

Затрагивает: API-схема `CampaignStructureIn` (поле `label: str | None`, сборка имён в
`_campaign_blocks`), фронт шаг 4 (поле «Метка»), типы `CampaignStructure`.
