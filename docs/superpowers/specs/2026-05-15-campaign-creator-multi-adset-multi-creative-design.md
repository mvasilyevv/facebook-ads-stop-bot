...
## Идемпотентность шагов: глобальное правило

Каждый шаг (кроме явных исключений ниже) перед выполнением проверяет текущее состояние UI. Если значение уже совпадает с целевым — шаг ничего не делает и возвращает `StepResult(skipped=True)`. Это критично для:
- Resume после сбоя (повтор уже частично выполненного шага не должен ломать состояние).
- Recorder-driven dev-флоу (запуск пайплайна на уже частично заполненной форме).

**Исключения — шаги, которые ВСЕГДА выполняются:**
- `upload_creatives` — креативы загружаем всегда (фактический файл на диске может отличаться от того, что прикреплено в FB).
- `reattach_creative` — то же.
- `duplicate_ad`, `duplicate_adset` — по природе создают новые сущности.

**Особый случай — `set_geo`:**
По умолчанию FB подставляет в Locations набор стран (исторически — «Китай», сейчас в записи 2026-05-15 — «Гонконг»; FB меняет дефолты со временем и регионально). Шаг должен:
1. Сначала прочитать текущий набор стран в инпуте.
2. Удалить ровно те, которых нет в `params.countries` (что бы там ни было — Китай, Гонконг, любая другая дефолтная).
3. Добавить недостающие из `params.countries`.

Не хардкодить «удалить Китай» или «удалить Гонконг» — диффим против `params.countries`.
