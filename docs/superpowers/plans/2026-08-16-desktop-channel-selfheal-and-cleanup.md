...
### Task 11: Второй источник того же шума — исход `skipped`

**Files:**
- Modify: `apps/observer_worker/main.py` (блок финализации исхода скана)
- Test: `tests/unit/test_observer_scan_action_lifecycle.py`

**Interfaces:**
- Consumes: `mark_cancelled(engine, *, task_id, reason, lease_owner, lease_token)` из `core/tasks/queue.py:1482` — та же функция, которой задача 7 финализирует исход `paused`.
- Produces: ничего для последующих задач.

**Откуда взялось:** ревью задачи 7 нашло, что тот же класс шума даёт второй исход. `run_one_cycle` возвращает `skipped`, когда сканирование ВКЛЮЧЕНО, но мониторить нечего — не настроено ни одного кабинета. Такая задача попадает в тот же `else` и записывается как «observer scan finished without a complete snapshot: skipped» с той же периодичностью ~45 секунд. Для оператора это неотличимо от отказа, хотя система просто не имеет предмета для работы.

Задача 7 закрыла путь публикации только для выключенного сканирования; здесь сканирование включено, поэтому лечение — на стороне финализации.

- [ ] **Step 1: Написать падающий тест**

В `tests/unit/test_observer_scan_action_lifecycle.py` рядом с тестом, который проверяет финализацию исхода `paused`, добавить такой же для `skipped`: задача финализируется как отменённая с собственной причиной, `mark_failed` не вызывается, fencing-тройка (`task_id`, `lease_owner`, `lease_token`) передаётся. Способ вызова и двойники возьми из соседнего теста — они там уже есть.

Причина отмены должна отличаться от `paused`: оператору важно различать «я выключил скан» и «сканировать нечего». Возьми `nothing_monitored`.

- [ ] **Step 2: Прогнать и убедиться, что падает**

Run: `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest tests/unit/test_observer_scan_action_lifecycle.py -q`
Expected: FAIL — `skipped` уходит в `mark_failed`.

- [ ] **Step 3: Отделить «нечего сканировать» от отказа**

В `apps/observer_worker/main.py` расширить ветку, которую задача 7 сделала для `paused`, вторым исходом. Причина отмены выбирается по исходу: `scanning_paused` для `paused`, `nothing_monitored` для `skipped`. Ветка провала остаётся для всех остальных исходов без изменений — это по-прежнему настоящие отказы, и их не надо прятать.

Комментарий должен объяснять, почему оба исхода не провал: в одном случае сработал выключатель оператора, в другом — нет предмета для работы; красное в обоих случаях означало бы поломку там, где её нет.
...
```bash
git add apps/observer_worker/main.py tests/unit/test_observer_scan_action_lifecycle.py
git commit -m "fix(observer): «сканировать нечего» — не отказ

Второй источник того же шума, найденный ревью: при включённом
сканировании и ненастроенных кабинетах скан возвращает skipped и с той же
периодичностью оседает в ленте как провал. Причина отмены отличается от
паузы: оператору важно различать «я выключил» и «нечего сканировать»."
```
...
- ✅ Релиз проходит зелёным без единого ручного действия между шагами — релиз
  `31958986440`: шаг `ensure_desktop_channel` отработал за 16 с, следом
  `verify_application` прошёл за 1,4 с вместо прежнего таймаута в 180 с.
- ✅ `/root/vision-channel-heal.sh` на прод-хосте отсутствует.
- ✅ Ни один текст в репозитории не описывает KasmVNC как часть образа —
  оставшиеся упоминания живут только в гард-тестах, которые нарочно называют
  снятый софт, чтобы он не вернулся, и в датированных материалах `docs/`.
- ✅ Оператор на странице стола видит, что нужно для подключения — адрес, ключ
  и ID с копированием. Требование VPN снято вместе с публикацией брокера:
  критерий писался до решения владельца и заменён на противоположный.

## Осталось за пределами этой ветки

- **Действие владельца:** открыть порты брокера в ufw. Публикации на `0.0.0.0`
  недостаточно: политика ufw на хосте — `deny (routed)`, и она режет внешний
  трафик к контейнерам независимо от пустой цепочки `DOCKER-USER`. Нужны
  `ufw route allow` на 21115-21117/tcp и 21116/udp. Проверено: с самого хоста
  порты отвечают, снаружи — нет.
- **Латентная ловушка (следствие двух изменений сразу):** барьер релиза в
  `core/observer/scan_tasks.py` требует, чтобы строка-зависимость существовала,
  а укреплённая ретенция теперь штатно удаляет терминальных детей. Барьер со
  смешанными зависимостями не разблокируется никогда. Продакшн-производителей
  `dependency_task_ids` сегодня нет, поэтому не горит.
- **Точность причины отмены:** исход `skipped` приходит из двух мест — «нет
  кабинетов» и money-гард «мульти-кабинет без owner_tag». Обе финализируются с
  причиной `nothing_monitored`, что для второго случая неточно. Сам алерт
  money-гарда идёт отдельным каналом и не затронут.
- **Мелочи из ревью:** `post_json` дублирует `patch_json` (риск расхождения
  обработки ошибок); нет контроллер-уровневого теста на падение шага канала;
  `operatorCommandTone` считает `cancelled` отказом, хотя ветка сделала его
  рутинным состоянием; уборка без `FOR UPDATE SKIP LOCKED`.
