...
**Отдельно про `ONLINE_GAMBLING_AND_GAMING`.** У Meta есть спец-категория ровно под тот
вертикаль, в которой работает проект, а мы зашили `NONE` и никогда её не объявляли. Это не
техническая мелочь: спец-категория меняет доступный таргетинг и правила модерации, а неверное
объявление — вопрос к рекламодателю, а не к коду. Решение владельца требуется до любой правки
(см. блок «Что требуется от тебя», пункт 4).
