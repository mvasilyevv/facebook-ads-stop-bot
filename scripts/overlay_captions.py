...
def _draw_centered(draw, cx, y, text, font, fill, stroke_fill, stroke_w, shadow=True):
    ...
    draw.text(
        (x, y), text, font=font, fill=fill, stroke_width=stroke_w * SS, stroke_fill=stroke_fill
    )
    return h


def build_overlay(
    w, h, top_text, bottom_lines, top_y=0.045, bottom_y=0.80, cover=False, full_width=False
):
    ...
    if bottom_lines:
        ...
        d.rounded_rectangle(
            ...
            radius=(0 if full_width else 22 * SS),
            fill=(8, 8, 8, bot_a),
        )
        if cover and full_width:
...
def main():
    ...
    ap.add_argument(
        "--cover", action="store_true", help="непрозрачные плашки (перекрыть вжатый текст)"
    )
