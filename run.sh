...
# ==========================================
# 2c. ENCRYPTION_KEY — единый bootstrap ДО старта воркеров
# ==========================================
# Money/security-критично: генерируем ключ РОВНО один раз под flock, пока воркеры
# ещё не запущены. Иначе 13+ параллельных процессов дописывали каждый свой ключ в
# .env → тихая порча зашифрованных токенов (канал авто-стопа слепнет).
echo -e "${BLUE}🔐 Проверяю ENCRYPTION_KEY...${NC}"
if ! .venv/bin/python scripts/ensure_encryption_key.py; then
    echo -e "${RED}❌ Не удалось гарантировать ENCRYPTION_KEY${NC}"
    exit 1
fi
# Подхватываем только что сгенерированный ключ в текущее окружение run.sh.
set -a
# shellcheck disable=SC1091
. ./.env
set +a
