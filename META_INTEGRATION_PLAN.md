...
**Уточнение по act-каналу (#39, 2026-05-29).** Решение «observer = DOM» относится к **detect** (scan, оценка правил — sub-минутная реакция критична). А **act** (disable/enable) — операция latency-tolerant: задержка в секунды на отключение уже-сработавшего STOP приемлема. Поэтому act вынесен под флаг `observer_config.act_via_api`: `True` → авто-стоп и ручные toggle-кнопки исполняются через `meta_api_mutation pause_ad/activate_ad` (точно по `ad_id`, не зависит от позиции кнопки/скролла/виртуализации — устраняет класс DOM-промахов), `False` → прежний DOM-клик. Live-замер (2026-05-29): 48 mutation-операций enable/disable, 0 промахов. Detect остаётся DOM в обоих режимах. FSM-консистентность гарантирует `core/meta_api/fsm_sync.py` (meta_api_worker приводит `ad_alert_state` к результату mutation).
