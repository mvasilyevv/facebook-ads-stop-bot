...
1. **observer_worker** (`apps/observer_worker/`) — бесконечный цикл: gRPC `RunScanCycle` → `ScannedAdRow[]` → process_scan_rows (FSM в `ad_alert_state` + метрики в `ad_metrics` partitioned + outbox в `task_queue`) → dispatch alerts в TG (через `core.telegram.alert_dispatcher`). **Owner-scoping:** если задан `observer_config.owner_campaign_tag` (один или несколько тегов через запятую, напр. `MV` или `MV,ABC`), кампании без любого из тегов (word-boundary, через `campaign_matches_owner` + `parse_owner_tags`) полностью игнорируются — защита от работы с чужими кампаниями в общем кабинете (NULL — фильтр выключен). **Канал авто-стопа**: только `meta_api_mutation pause_ad` (Marketing API через meta_api_worker, точно по ad_id). DOM-канал отключения (toggle-клик, флаг `act_via_api`, disable_worker) удалён; detect остаётся через `am_tabular`. Heartbeat и runtime status — в Redis (`observer:runtime` с TTL 60s). Pubsub `fb_agent:scan:finished`. **Прод gate-фабрика** (`_default_gate_factory`) собирает `BrowserAgentClient(BrowserAgentConfig)` из `get_settings()` + `start()` (ранее был баг: `BrowserAgentClient()` без config + `.connect()`). Точка входа: `run_observer_worker.py`.
2. **telegram_poller** (`apps/telegram_poller/`) — long-polling Telegram Bot API. Команды: `/start [code]` (consume invite), `/help`, `/spy <slot> <country>` (Ad Library pipeline), `/setup_topics` (owner-only — форум-топики супергруппы), `/topics` (раскладка). Inline-кнопки `dis:`, `snz:` под алертами → создают `task_queue` запись или ставят `ad_alert_state.snoozed_until`. Точка входа: `run_telegram_poller.py`.
3. **cleanup_worker** (`apps/cleanup_worker/`) — раз в сутки в 04:00 UTC: DROP старых партиций, DELETE по retention из `system_config.retention_policy`, чистка orphan ad_library media файлов, CREATE next-month партиций. Точка входа: `run_cleanup_worker.py`.
...
- **`apps/api/routers/v1/`** — пакет с auto-discovery. `register_all(app)` через `pkgutil.iter_modules` находит все модули с атрибутом `router: APIRouter` и подключает их с `prefix="/api"`. Новые роутеры просто кладутся в эту папку — без правок `main.py`. После Round 7.1-7.7 закрыт 61 endpoint в 17 модулях:
  - **Round 7.1 — Settings + Observer + Health:**
    ...
    - `health_details.py` — `GET /health/details` (SCAN MATCH `worker:heartbeat:*` → ONLINE/OFFLINE, overall HEALTHY/DEGRADED/CRITICAL). Секция `meta_api_channel` — читает Redis `meta_api:channel:health` (пишет health_watchdog probe): ONLINE (канал жив) / DEGRADED (network-down → понижает overall до DEGRADED) / UNKNOWN (нет прободера/ключ протух → overall НЕ понижает). Роутер только читает Redis, не зависит от browser-agent.
  - **Round 7.2 — Offers:**
...
- 8200+ строк TypeScript, gRPC порт 50051, три service'а:
  ...
  - `ScannerService` — детект объявлений (RunScanCycle stream, HardReloadPage, ListCampaigns). DOM-toggle (ToggleAd/HumanMove) и часть DOM-scan RPC удалены при переходе отключения на Marketing API; детект — через `am_tabular`.
...
- **telegram/** — `client.py` (TG Bot API через httpx, не зависит от ORM), `service.py` (load_telegram_config, find_recipient, consume_invite), `bot_handler.py` (минимальный: /start /help /spy + callback'и под алертами), `renderer.py` (форматирование алертов с inline-кнопками `dis:`/`snz:`), `alert_dispatcher.py` (pre-claim паттерн: `INSERT ... ON CONFLICT DO NOTHING RETURNING id` с sentinel `message_id=0` ДО `sendMessage` → при NULL skip без send'а, иначе send + UPDATE реальным id, при ошибке DELETE pre-claim для retry; защита от дубля TG-сообщений в SELECT/INSERT race), `messaging.py`, `format.py` (общие HTML-форматтеры «чистая карточка»: `esc`/`b`/`code`/`money`/`table`/`kv_grid` — единый источник для всех рендереров, parse_mode=HTML по умолчанию), `topics.py` (идемпотентный провижн форум-топиков супергруппы + `PgTopicStore`; маршрутизация по стадии `warning`/`stop`/`enable`/`ops`/`digest`), `handlers/topics.py` (`/setup_topics` — owner-only создание топиков, `/topics` — раскладка).
...
- **ai_assistant/** — pure-Python ассистент: `chat.py`, `client.py`, `providers.py`, `prompts/`. Пакет `tools/` (registry + base + ops/meta/drafts/creative — 15 tools). **AI-ассистент в Telegram удалён** (`/ask` + `core/telegram/handlers/ask.py` снесены): read-only tools доступны только через MCP-сервер; write-tools (`drafts/request_*`, DRAFT_REQUIRED) в MCP отключены. Web-канал `POST /ai/analyze` зовёт `ChatSession(allow_tools=False)` — без инструментов. `ToolHandler.risk_level`: READ_ONLY (исполняется немедленно), DRAFT_REQUIRED (создаёт `task_queue` со `status='draft'` + `created_by_chat_id` через `core.meta_api.queue.create_draft_task`; подтверждение — owner или recipient `role='owner'`; в проде создаётся ручными `/pause` `/resume`), CREATIVE. Rate-limit per `client_key` через `tools/_ratelimit.py` (Redis `ai:ratelimit:tools:*` TTL 3600 + in-memory secondary cap 5/60с при сбое Redis вместо fail-open). `MetaApiClient` пробрасывается через `ToolContext` — без него meta-tools падают с явной ошибкой. `drafts/request_bulk_pause` ищет по `offer_code` через Postgres regex `~*` с anchored word-boundary `(^|[^a-z0-9])CODE([^a-z0-9]|$)` — старая ILIKE-substring выборка (`%CR%` матчит `ACRO`) удалена.
...
- **Frontend ↔ backend shape расхождения** (зафиксированы при восстановлении API Round 7.2-7.3). 5 кандидатов из этого списка проверены по фактическому использованию в новом фронте `frontend/` и закрыты в **BL-12** (коммит `c870b4a`): добавляли колонку только тем полям, что фронт реально читает, остальное — осознанный null-маппинг/alias. Мёртвые поля (объявлены лишь в TS-типах, в JSX не читаются) в БД не заводили.
  - ~~`Offer` нет полей `country_code`, `use_vision_creator`, `notes`~~ — ✅ BL-12: новый фронт (`OfferFormModal`) использует только `code/name/vertical/is_active`; эти три поля — мёртвые (только в `api-generated.ts`). Оставлен null-маппинг в `OfferOut`, вход в `OfferCreateIn`/`OfferUpdateIn` принимается и игнорируется. Колонки НЕ добавляем.
  - ~~`OfferRule` ждёт JSONB `cpc_thresholds`/`cpl_thresholds`~~ — ✅ BL-12: расхождение было фиктивным. Новый фронт (`RulesForm`) использует 6 числовых порогов (`spend_no_event_threshold`/`cpa_threshold`/`cpm_threshold`/`ctr_threshold`/`frequency_threshold`/`funnel_ratio_threshold`), которые УЖЕ есть в схеме; имён `cpc_thresholds`/`cpl_thresholds` во `frontend/src/` нет вообще.
  - ~~`AdMetrics` нет `delivery_status`~~ — ✅ BL-12 (migration `0010_fb_ads_delivery_status`): колонка живёт в каталоге `fb_ads` (low-cardinality текущее состояние, не раздуваем partitioned `ad_metrics`), заполняется `writers.upsert_catalog_hierarchy` из `ScannedAdRow.delivery_status` на каждом скане, отдаётся `snapshot.py` → `AdSnapshotOut.delivery_status`. Meta-канал маппит `effective_status` → `delivery_status` в `adapters.py`. Фронт пока поле не рендерит — доступно когда понадобится.
  - ~~`AlertEvent` нет `triggered_by_rule_codes`~~ — ✅ BL-12: alias на `matched_rule_codes` (единая точка `apps/api/utils/alert_serializer.py`, плюс `AdSnapshotOut`/`AlertEventOut`/`ads_timeline`). В UI фронт читает `matched_rule_codes` — отдельная колонка не нужна.
  - ~~`AdAlertState` нет отдельных `last_warning_at`/`last_stop_at`~~ — ✅ BL-12: `snapshot.py` отдаёт **реальные** времена из append-only `alert_events` (LATERAL `ev_stages`: `MAX(created_at) FILTER (WHERE stage='warning'/'stop')`), а НЕ CASE-реконструкцию из `current_stage` (старый подход терял `last_warning_at` при эскалации warning→stop). Отдельные колонки в FSM-таблицу НЕ добавляем — это денормализация append-only лога.
...
- **Subscriber'ы в worker'ах:** observer подписан на `fb_agent:observer:trigger` (scan-now прерывает sleep → немедленный скан), `cabinet_day` (форс-рескан нового дня; архив делает эндпоинт-публишер, observer не дублирует), `fb_agent:worker:restart:observer`; disable/enable слушают restart.
...
Адаптирует AI-tools в формат `mcp.types.Tool`, но **только read-only** (ops/meta/trackers/creative): DRAFT_REQUIRED (`request_*`) исключены из `list_tools` и отклоняются в `call_tool` — write-мутации через MCP недоступны (подтверждение AI-черновиков убрано).
...
> ⚠️ Возможно устарело: детект объявлений переведён на `am_tabular` (Marketing API через `page.evaluate(fetch)`, `services/browser-agent/src/am/am-fetch.ts`). Описание DOM-парсинга ниже — историческое; проверить актуальность при работе со сканером.
...
**Web (`frontend/`, пакет `fb-stop-bot-frontend`):** React 19 + Vite 6 + **TypeScript strict** + Tailwind 4 (`@theme`) + TanStack Router (file-based) + TanStack Query 5 + Zustand 5 + Lucide + Radix Primitives. Dark-only, monochrome editorial style (см. `docs/frontend_design.md`). Desktop 1280+. **6 страниц:** Dashboard, Ads (+ drawer деталей), Campaigns (owner-tag + allowlist), Offers, History, Settings (табы Observer/Telegram/Vision/Health). Страница Drafts удалена (подтверждение AI-черновиков убрано). Виртуализованная таблица (`@tanstack/react-virtual`). WS live-invalidation (backoff + polling). Storybook 8. ~331 vitest-тест. Порт dev/preview **5174**, proxy `/api` → `:8100`.

**Telegram Mini App (`frontend-mini/`, пакет `fb-agent-mini`):** React 19 + Vite 6 + **TypeScript** + Tailwind 4 + TanStack Router/Query. Мобильная адаптация (нижний tab-bar 4 вкладки, safe-area, тач ≥44px). **8 экранов:** Dashboard, Ads, Ad Detail, Health, History, Offers, Scripts, Settings (экран Drafts удалён). TMA auth (initData → Bearer). ~89 vitest-тестов. Порт dev **5175**, base `/tma/`. Форматтеры и FSM-константы берутся из `@fb/shared` (дублирование устранено).
...
- **Draft-first для write-мутаций.** Write-операции создают запись в outbox со статусом `DRAFT`, человек подтверждает inline-кнопкой ✅/❌ в Telegram под превью ручных `/pause` `/resume` (`core/telegram/handlers/draft_confirm.py`), затем worker исполняет. Reconciler чистит протухшие DRAFT через 24 часа. AI-ассистент, ранее создававший такие черновики из `/ask`, удалён.
- **Отключение объявления — два пути:** (1) автоматически — observer создаёт `meta_api_mutation pause_ad` при STOP-правиле (`requested_by="bot_auto_stop"`); (2) вручную — пользователь через TG (`/ads` или inline-кнопка под алертом). В обоих случаях через `meta_api_worker` (Marketing API `pause_ad`, точно по ad_id); auto-задача отменяется reconcile-логикой, если объявление успело уйти из STOP до клика. DOM-канал (toggle-клик, disable_worker) удалён.
- **FSM однонаправленная:** повторный WARNING после WARNING_SENT не дублируется, эскалация WARNING_SENT → STOP_SENT возможна. Обратные сбросы из STOP_SENT/CLAIMED/DISABLED разрешены только observer'у (`reopen_reactivated_alert_state`) и enable-пути через Marketing API (`activate_ad` → `core/meta_api/fsm_sync.py::sync_fsm_after_mutation` сбрасывает alert_state в NORMAL).
