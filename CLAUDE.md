...
- **`apps/api/routers/v1/`** — пакет с auto-discovery. `register_all(app)` через `pkgutil.iter_modules` находит все модули с атрибутом `router: APIRouter` и подключает их с `prefix="/api"`. Новые роутеры просто кладутся в эту папку — без правок `main.py`. После Round 7.1-7.5 закрыты 50 endpoints в 14 модулях:
