...
1. **observer_worker** (`apps/observer_worker/`) — бесконечный цикл: gRPC `RunScanCycle` → `ScannedAdRow[]` → process_scan_rows (FSM в `ad_alert_state` + метрики в `ad_metrics` partitioned + outbox в `task_queue`) → dispatch alerts в TG (через `core.telegram.alert_dispatcher`). **Owner-scoping:** если задан `observer_config.owner_campaign_tag` (напр. `MV`), кампании без этого тега (word-boundary, через `campaign_matches_owner`) полностью игнорируются — защита от работы с чужими кампаниями в общем кабинете (NULL — фильтр выключен). Heartbeat и runtime status — в Redis (`observer:runtime` с TTL 60s). Pubsub `fb_agent:scan:finished`. **Прод gate-фабрика** (`_default_gate_factory`) собирает `BrowserAgentClient(BrowserAgentConfig)` из `get_settings()` + `start()` (ранее был баг: `BrowserAgentClient()` без config + `.connect()`). Точка входа: `run_observer_worker.py`.
