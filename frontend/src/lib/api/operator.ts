...
export async function fetchOperatorSnapshotForRealtime(queryClient: QueryClient) {
  ...
  // active account/window variant shown by the UI must complete a post-event
  ...
  const hasActiveSnapshots =
    queryClient.getQueryCache().findAll({ queryKey: snapshotKey, type: "active" }).length > 0;
  ...
  if (snapshots.length === 0) {
    throw new Error("active operator snapshots disappeared during reconciliation");
  }
