...
vi.mock("@/components/ui/toastStore", () => ({ toast: mockToastStore }));
  ...
  return render(
    <QueryClientProvider client={client}>
      <OperatorRealtimeStatusProvider status="connected">
        <OperatorCabinetDashboard cabinetId={cabinetId} />
      </OperatorRealtimeStatusProvider>
    </QueryClientProvider>,
  );
}

describe("operator cabinet page", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("uses the selected cabinet timezone and does not show the portfolio toggle", () => {
    renderCabinet();
    ...
    // Портфель (список всех кабинетов) и воронка — не про один кабинет.
  ...
  it("shows the cabinet's spend/base/stop line with a server-confirmed overage note", () => {
    ...
    expect(within(section).getByText("$18.00")).toBeInTheDocument();
  ...
  it("renders this cabinet's ads with a preset account_id filter, reusing the shared ads list", () => {
  ...
  it("queues pause from the cabinet page with the exact same request shape as /ads", async () => {
