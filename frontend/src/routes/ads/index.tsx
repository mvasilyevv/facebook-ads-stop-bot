...
function AdsPage() {
  ...
  // проецируется отдельно, как и в /actions, чтобы дисконнект realtime не
  // потерялся при склейке. Самая свежая порция (последняя) даёт честные
  ...
  // видит только загруженные страницы и переупорядочивать их не должен.
  ...
  return (
    ...
    >
      ...
      <Select
        ...
        onChange={(value) => onChange({ sort: value as OperatorAdsRouteSort })}
      >
        {SORTS.map((item) => (
