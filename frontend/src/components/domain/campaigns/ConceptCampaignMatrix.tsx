...
const CAMPAIGN_COLORS = [
  {
    ...
    chip: "border-blue-500/40 text-blue-300 hover:bg-blue-500/15",
  },
  {
    dot: "bg-purple-400",
    ring: "border-purple-500/40",
    soft: "bg-purple-500/[0.07]",
    chip: "border-purple-500/40 text-purple-300 hover:bg-purple-500/15",
  },
  {
    dot: "bg-emerald-400",
    ring: "border-emerald-500/40",
    soft: "bg-emerald-500/[0.07]",
    chip: "border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/15",
  },
  {
    dot: "bg-amber-400",
    ring: "border-amber-500/40",
    soft: "bg-amber-500/[0.07]",
    chip: "border-amber-500/40 text-amber-300 hover:bg-amber-500/15",
  },
  {
    dot: "bg-pink-400",
    ring: "border-pink-500/40",
    soft: "bg-pink-500/[0.07]",
    chip: "border-pink-500/40 text-pink-300 hover:bg-pink-500/15",
  },
  {
    dot: "bg-cyan-400",
    ring: "border-cyan-500/40",
    soft: "bg-cyan-500/[0.07]",
    chip: "border-cyan-500/40 text-cyan-300 hover:bg-cyan-500/15",
  },
];
...
export const ConceptCampaignMatrix: FC<ConceptCampaignMatrixProps> = ({
  ...
  // Сводки.
  const poolImg = concepts.filter((c) => !isVideo(c.content_type ?? null)).length;
  const poolVid = concepts.filter((c) => isVideo(c.content_type ?? null)).length;
  ...
  return (
    <>
      ...
      {concepts.length > 0 && campaigns.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          <div className="flex items-center gap-2 text-[12px] text-bg-9">
            <span className="font-display text-[12px] tracking-[0.14em] uppercase text-bg-8">
              Загружено
            </span>
          ...
          <div className="flex items-center gap-2 ml-auto">
            <span className="text-[12px] text-bg-8">Распределить:</span>
            ...
            <button
              type="button"
              onClick={clearAll}
              className="min-h-11 px-3 text-[12px] text-bg-8 transition-colors hover:text-bg-10"
            >
              Очистить
            </button>
          </div>
      ...
      {campaigns.length > 0 && unassigned.length > 0 && (
        <div className="rounded-[var(--radius-3)] border border-dashed border-[var(--color-hairline-strong)] bg-bg-1 p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[12px] text-bg-8">— нажмите кампанию, чтобы добавить</span>
          </div>
...
const DistributeButton: FC<{ onClick: () => void; children: React.ReactNode }> = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className="min-h-11 rounded-[var(--radius-2)] border border-[var(--color-hairline-strong)] px-3 font-display text-[12px] text-bg-10 transition-colors hover:border-accent hover:text-accent"
  >
    {children}
  </button>
);
...
const PoolCard: FC<PoolCardProps> = ({ concept, campaigns, colorByKey, onAssign, onDelete }) => {
  const video = isVideo(concept.content_type ?? null);
  return (
    <div className="flex items-center gap-2 rounded-[var(--radius-2)] border border-[var(--color-hairline)] bg-bg-2 pl-2 pr-1.5 py-1.5">
      <span
        className={cn(
      ...
      <span className="text-[12px] text-bg-11 truncate max-w-[120px]" title={concept.original_name}>
        ...
        {campaigns.map((c) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex min-h-11 min-w-11 items-center gap-1 rounded border bg-bg-1 px-2 font-display text-[12px] transition-colors",
                color.chip,
              )}
            >
      ...
      <button
        ...
        onClick={onDelete}
        className="flex size-11 shrink-0 items-center justify-center rounded-[var(--radius-2)] text-bg-8 transition-colors hover:bg-danger/10 hover:text-danger"
      >
        <Trash2 size={12} />
      </button>
    </div>
...
const CampaignColumn: FC<CampaignColumnProps> = ({
  ...
  const img = attached.filter((c) => !isVideo(c.content_type ?? null)).length;
  const vid = attached.filter((c) => isVideo(c.content_type ?? null)).length;
  ...
  return (
    <div className={cn("rounded-[var(--radius-3)] border bg-bg-1 flex flex-col", color.ring)}>
      ...
      <div
        className={cn(
          "px-3 py-2.5 border-b border-[var(--color-hairline)] rounded-t-[var(--radius-3)]",
          color.soft,
        )}
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-bg-8 shrink-0">· {campaign.adset_count} adset</span>
        </div>
      </div>
      ...
      <div className="p-2 space-y-1.5">
        {attached.length === 0 ? (
          <div className="flex items-center gap-2 text-[12px] text-amber-400/90 px-1 py-2">
        ...
        {pool.length > 0 && (
          <div>
            <button
              ...
              className={cn(
                "flex min-h-11 w-full items-center justify-center gap-1.5 rounded-[var(--radius-2)] border border-dashed px-3 text-[12px] transition-colors",
                picking
                  ? "border-accent text-accent bg-accent-bg/40"
                  : "border-[var(--color-hairline-strong)] text-bg-8 hover:text-bg-11 hover:border-accent",
              )}
            >
              <Plus size={13} />
              добавить
            ...
            {picking && (
              <div className="mt-1.5 space-y-1 rounded-[var(--radius-2)] border border-[var(--color-hairline)] bg-bg-2 p-1.5">
                <div className="text-[12px] uppercase tracking-wider text-bg-8 px-1 pb-0.5">
                  Доступно ({pool.length})
                </div>
                {pool.map((c) => {
                  const video = isVideo(c.content_type ?? null);
                  return (
                    <button
                      key={c.ref}
                      type="button"
                      onClick={() => onAttach(c.ref)}
                      className="flex min-h-11 w-full items-center gap-2 rounded-[var(--radius-1)] px-2 text-left transition-colors hover:bg-bg-3"
      ...
      <div className="px-3 py-2 border-t border-[var(--color-hairline)] mt-auto">
        {attached.length > 0 ? (
          ...
          <div className="text-[12px] text-bg-8">0 объявлений</div>
        )}
      </div>
    </div>
  );
...
const ConceptCard: FC<ConceptCardProps> = ({ concept, onRemove, removeLabel }) => {
  const video = isVideo(concept.content_type ?? null);
  return (
    <div className="flex items-center gap-2 rounded-[var(--radius-2)] border border-[var(--color-hairline)] bg-bg-2 px-2 py-1.5">
      <span
        className={cn(
          "size-6 shrink-0 rounded-[var(--radius-1)] flex items-center justify-center",
          video ? "bg-purple-500/10 text-purple-400" : "bg-blue-500/10 text-blue-400",
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[12px] text-bg-8">{formatBytes(concept.size_bytes)}</div>
      </div>
      <button
        type="button"
...
const PoolRow: FC<{ concept: UploadedConcept; onRemove: () => void }> = ({ concept, onRemove }) => {
  const video = isVideo(concept.content_type ?? null);
  return (
    <div className="border border-[var(--color-hairline)] rounded-[var(--radius-2)] px-3 py-2.5 bg-bg-1 flex items-center gap-3">
      <div
        className={cn(
